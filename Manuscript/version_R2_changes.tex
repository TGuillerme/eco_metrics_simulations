...
%DIF DEL main_ecography_R1.tex   Sat Nov 30 13:31:48 2024
%DIF ADD main_ecography_R2.tex   Fri Jan 24 10:26:16 2025
\usepackage{natbib}
...
\DIFdelbegin \DIFdel{While the first part of this framework (``what?'') has been studied extensively in the last decade, this was often undertaken outside of the ``what, how and why'' framework.Here }\DIFdelend \DIFaddbegin \DIFadd{Although some previous work has been done in understanding patterns in the context of mechanisms and process (e.g. \mbox{%DIFAUXCMD
...
\caption{\DIFaddFL{Glossary and equivalence of terms used in this papers; adapted from \mbox{%DIFAUXCMD
...
\hline
\DIFaddFL{Trait space }& \DIFaddFL{Matrix ($n \times d$) with a structural relation between rows and columns }& \DIFaddFL{Functional space, morphospace , etc. }& \DIFaddFL{Morphospace, traitspace, etc.  }\\
\hline
...
\hline
\DIFaddFL{Traits }& \DIFaddFL{Columns subset ($b \times n$; $b \leq d$) }& \DIFaddFL{uni/multidimensional characteristic*; e.g. geographical location }& \DIFaddFL{uni/multidimensional characteristic; e.g. landmark coordinates }\\
\hline
...
\hline
\DIFaddFL{Stressors' intensity }& \DIFaddFL{The amount of observation removed (percentages) }& \DIFaddFL{Proportion of species affected, proportion of temperature change, etc. }& \DIFaddFL{Severity of mass extinction,  etc. }\\
\hline
\end{tabular}
...
's suggestions which is expanded to include traits beyond the ones used for functional ecology (macroevolution, palaeontology, microbiology, etc.).
}\end{table}
...
First, we simulated \DIFdelbegin \DIFdel{a single }\DIFdelend \DIFaddbegin \DIFadd{multiple independent }\DIFaddend random time dependent trait (Brownian Motion) under a model where lineages only speciate (no extinction; i.e. a pure birth speciation model) until reaching 200 observations in \texttt{R} \citep{rcore} using \texttt{treats} \citep{guillerme2024treats}.
...
\DIFdelbegin \DIFdel{I.e. }\DIFdelend \DIFaddbegin \DIFadd{In other words, }\DIFaddend pairs of observations that share similar trait combinations are more \DIFdelbegin \DIFdel{likely to remain }\DIFdelend \DIFaddbegin \DIFadd{common in the trait space than observations with dissimilar trait combinations }\DIFaddend (i.e. observations adjacent to each other in the trait space \DIFaddbegin \DIFadd{are more likely compared to common ones}\DIFaddend ).
...
\hline
Regularity & Trait Probability Density & Functional evenness & An index of the spread of the trait distribution in an even spread of the distribution (e.g. the height of bins in a histogram) & \texttt{TPD::REND}\DIFaddbeginFL \DIFaddFL{* }\DIFaddendFL & \cite{carmona2019trait}\\
 & \cr 
...
\label{Tab:metrics}
\DIFaddbeginFL \DIFaddFL{* The }\texttt{\DIFaddFL{TPD}} \DIFaddFL{package currently only allows calculations for traitspaces up to four dimensions. This metric was exclude from traitspaces with more than four dimensions.
...
\DIFaddbegin \DIFadd{Note also that, as mentioned above, different dimensionalities can have different effects on the metrics, depending on both the traitspace and the metric nature \mbox{%DIFAUXCMD
...
This }\DIFaddend could be due to the \DIFdelbegin \DIFdel{fact that although most global bird species are (or were, if accounting for anthropogenic extinctions) passerines (}\DIFdelend \DIFaddbegin \DIFadd{taxonomic distribution of the extinct birds: this example data has }\DIFaddend approximatively $58$\% \DIFdelbegin \DIFdel{), }\DIFdelend \DIFaddbegin \DIFadd{passerines while }\DIFaddend the majority of \DIFdelbegin \DIFdel{global known anthropogenic extinctions (approx. }\DIFdelend \DIFaddbegin \DIFadd{anthropogenic extinctions have been affecting non-passerines (approximatively }\DIFaddend $75$\%)\DIFdelbegin \DIFdel{have been non-passerines, which }\DIFdelend \DIFaddbegin \DIFadd{.
These species }\DIFaddend tend to be larger and possess more unique morphological traits (thus resulting in relatively large reductions in functional diversity).
