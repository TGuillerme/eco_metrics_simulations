...
Some metrics even captured an opposite pattern to what was expected: for all metrics, we naively expected a relative decrease in trait space occupancy following different levels of species removals but some metrics captured an \textit{increase} of trait space occupancy (e.g. the functional evenness based on hypervolume for the equalising mechanism) or non-linear ones (e.g. Alpha diversity based on dendrogram for the equalising mechanism).
