...
We chose this algorithm to approximate facilitation mechanism where a stressor could increase the probability of extinction for observations that are more isolated in the trait space.
...
We chose this algorithm to approximate a flitering assuming that a stressor that could increase the probability of extinction for observations further from some trait combination optimum.
...
This could approximate the fact that observation that have similar trait combinations are more likely to go extinct, due to competition (e.g. using the same resources). 
\end{itemize}
