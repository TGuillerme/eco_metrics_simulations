...
%DIF ADD main_ecography_R1.tex   Sat Nov 30 13:31:48 2024
\usepackage{natbib}
...
Through a simulation exercise, we analysed different patterns (what) across different processes (how) approximating different mechanisms (why) of interaction between organisms: equalisation, filtering, facilitation and competition (Figure \ref{Fig:simulations} \DIFaddbegin \DIFadd{and Table \ref{Tab:mechanisms}).
Our goal is to test the relative performance of different metrics to capture the patterns of different mechanisms by comparing the score of various metrics under specifically approximated mechanisms and the absence of any specific mechanism (null mechanism}\DIFaddend ).
...
Note here that we refer to traits as any measurable aspect of an organism (}\textit{\DIFadd{sensu}} \DIFadd{\mbox{%DIFAUXCMD
\cite{mcgill2006rebuilding} }\hskip0pt%DIFAUXCMD
- but see \mbox{%DIFAUXCMD
\cite{dawson2021traits} }\hskip0pt%DIFAUXCMD
for nuances in what researchers perceive).
...
These observations on which the traits are measured can represent any biological focal entities, e.g. tips in a phylogenetic tree}\DIFaddend , individuals, species, \DIFaddbegin \DIFadd{populations, }\DIFaddend OTUs, etc. \DIFdelbegin \DIFdel{) in }\texttt{\DIFdel{R}} %DIFAUXCMD
\DIFdel{\mbox{%DIFAUXCMD
\citep{rcore} }\hskip0pt%DIFAUXCMD
using }\texttt{\DIFdel{treats}} %DIFAUXCMD
\DIFdel{\mbox{%DIFAUXCMD
\citep{guillerme2024treats}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{(see Glossary in the supplementary materials, table 1)}\DIFaddend .
...
\item Increasing density: by removing the required amount of pairs of points with a \DIFdelbegin \DIFdel{pair }\DIFdelend \DIFaddbegin \DIFadd{variable pairwise }\DIFaddend distance of at least $D$\DIFdelbegin \DIFdel{(i.e.
removing the }\DIFdelend \DIFaddbegin \DIFadd{.
...
\DIFaddbegin \DIFadd{We chose this algorithm to approximate the facilitation mechanism where a stressor could increase the probability of extinction for observations that are more isolated in the trait space.
...
\DIFaddbegin \DIFadd{We chose this algorithm to approximate a flitering mechanism assuming a stressor that could increase the probability of extinction for observations further from some trait combination optimum.
...
e. an estimated number of categories summarising the distribution , like in a histogram).
...
This is done by selecting $B$ discrete categories to summarise the distribution - i.e. bandwidths - using Silverman's ``rule of thumb'' \mbox{%DIFAUXCMD
\citep[\texttt{bw.nrd0()} function in \texttt{R};][]{silverman1986density}}\hskip0pt%DIFAUXCMD
.
Each discrete category $B$ }\DIFaddend has an observed probability of sampling of \DIFdelbegin \DIFdel{$n$ (proportional to the number of elements in that bandwidth) and is skewed to }\DIFdelend \DIFaddbegin \DIFadd{$b$ (the proportion of observations in the category) and each observation then gets }\DIFaddend a probability of \DIFdelbegin \DIFdel{sampling of $n \times m^{p}$ where $m>0$ for the low values of $n$ and $m<0$ for high values of $n$ (respectively low and high density bandwidths) for flattening the curve (and the opposite for steepening it) and }\DIFdelend \DIFaddbegin \DIFadd{resampling scaling with the intensity of the stressor (the proportion of what to remove $i$) of $i \times (1-b)^{p}$. 
Where }\DIFaddend $p$ is a factor increasing the \DIFdelbegin \DIFdel{flattening/steepening }\DIFdelend \DIFaddbegin \DIFadd{scaling power of the algorithm }\DIFaddend (here we used $p=3$).
...
}\DIFaddend This approximates our \textbf{competition mechanism}\DIFaddbegin \DIFadd{, where observations in dense regions of the trait space are more likely to be removed than in sparse regions of the trait space.
...
This could approximate the fact that observations that have similar trait combinations are more likely to go extinct due to competition (e.g. using the same resources)}\DIFaddend . 
\end{itemize}
...
\DIFaddbegin \DIFadd{Note that the algorithm used here to simulate the stressors depends, to some extent, on the distribution of the data for each trait space.
...
We repeated each simulation pipeline \DIFaddbegin \DIFadd{for 4 dimensional traits }\DIFaddend (generate a trait space and apply the stressor \DIFdelbegin \DIFdel{) for uncorrelated 4 dimensional traits (}\DIFdelend \DIFaddbegin \DIFadd{- }\DIFaddend Figure \ref{Fig:simulation_results})\DIFdelbegin \DIFdel{and uncorrelated }\DIFdelend \DIFaddbegin \DIFadd{, and for }\DIFaddend 2 and 8 dimensional \DIFdelbegin \DIFdel{traits (supplementary materials).
}\DIFdelend \DIFaddbegin \DIFadd{(see supplementary materials figures 1 and 2).
In each pipeline, the dimensions where simulated as uncorrelated.
}\DIFaddend We limited our simulations to a relatively small number of \DIFdelbegin \DIFdel{traits }\DIFdelend \DIFaddbegin \DIFadd{dimensions }\DIFaddend due to the constraints of some of the metrics used (e.g., \texttt{TPD::TPDsMean} is only implemented for up to 4 dimensions; \citealt{carmona2019trait} \DIFaddbegin \DIFadd{- note that in the supplementary figures, we measured the }\texttt{\DIFadd{TPD}} \DIFadd{metrics on only the four first dimensions}\DIFaddend ) but also to avoid dealing with the curse of dimensionality \citep{bellman1957dynamic}.
This curse changes the properties of space occupancy in a non-linear way depending on \DIFdelbegin \DIFdel{the element}\DIFdelend \DIFaddbegin \DIFadd{each observation}\DIFaddend 's distribution and the number of dimensions.
...
. }& \DIFaddFL{Metrics mainly capturing changes in richness (but less in divergence and not in regularity) }\DIFaddendFL \\
  ...
  \DIFaddendFL Facilitation & Density change (``density'') & When \DIFdelbeginFL \DIFdelFL{species with closely shared }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{observations with similar }\DIFaddendFL trait combinations (i.e. located close together in the trait space) are more likely to survive the stressor \DIFaddbeginFL \DIFaddFL{\mbox{%DIFAUXCMD
...
) using 12 metrics (Table \ref{Tab:metrics})}\DIFaddend :
...
Effectively, for each metric and each single simulation, we subtracted the results under }\DIFdelend \DIFaddbegin \DIFadd{understand the ability of each metric to capture a mechanism of interest, we compared their scores relative to the same metric scores of a null mechanism.
To ease interpretation of the results across a range of metrics with different orders of magnitudes of outputs and different random simulations, we first calculated the difference between the stressor of interest and }\DIFaddend the null stressor \DIFdelbegin \DIFdel{to the ones under the other stressors and divided the resulting centred metrics by the maximum value for each metric between each simulation.}\DIFdelend \DIFaddbegin \DIFadd{(e.g. $metric_{stressor20\%} - metric_{null20\%}$).
...
}\DIFaddend This resulted in each metric being scaled between \DIFdelbegin \DIFdel{–1 and 1 where 0 corresponds to no change compared to }\DIFdelend \DIFaddbegin \DIFadd{$-1$ and $1$ where a negative difference can be interpreted as the random removal of elements leading to a higher metric score; a positive difference as the focal removal of elements (i.e. approximating equalisation, facilitation, filtering and competition) leading to a higher metric score; and a difference near $0$ meaning both the null stressor and }\DIFaddend the \DIFdelbegin \DIFdel{null and 1 and –1 the maximum positive and negative deviations relative to the null.
In parallel, we also compared the distribution of the 171 }\DIFdelend \DIFaddbegin \DIFadd{focal stressor having a similar effect on the metric score (Figs \ref{Fig:simulations}, \ref{Fig:simulation_results} and \ref{Fig:empirical_results}).
...
Finally, we measured the effect of the different \DIFdelbegin \DIFdel{levels of data removal (}\DIFdelend \DIFaddbegin \DIFadd{strengths of the stressors (i.e. removing }\DIFaddend 20\%, 40\%, 60\% and 80\% \DIFaddbegin \DIFadd{the data}\DIFaddend ) by fitting a linear regression using the \DIFdelbegin \DIFdel{model centred and scaled metric score $\sim$ }\DIFdelend \DIFaddbegin \DIFadd{scaled metric difference to the null score as a function of the }\DIFaddend amount of data removed
\DIFdelbegin \DIFdel{.
}\DIFdelend For each of these models, we reported the \DIFdelbegin \DIFdel{model fit }\DIFdelend \DIFaddbegin \DIFadd{variance explained by the model }\DIFaddend (adjusted $R^2$) in the supplementary tables \DIFdelbegin \DIFdel{1 to 3 }\DIFdelend \DIFaddbegin \DIFadd{$2$ to $5$ }\DIFaddend and whether the slope was clearly distinguishable from \DIFdelbegin \DIFdel{0 }\DIFdelend \DIFaddbegin \DIFadd{$0$ }\DIFaddend (Figure \ref{Fig:simulation_results}\DIFdelbegin \DIFdel{).
}\DIFdelend \DIFaddbegin \DIFadd{; }\textit{\DIFadd{sensu}} \DIFadd{\mbox{%DIFAUXCMD
\citealt{dushoff2019can}}\hskip0pt%DIFAUXCMD
).
...
$55$ }\DIFaddend species are known to have gone extinct prior to \DIFdelbegin \DIFdel{1500 CE and 26 after 1500 CE.
}\DIFdelend \DIFaddbegin \DIFadd{$1500$ CE and $26$ after $1500$ CE.
Note that we used the extinct species data as presented in \mbox{%DIFAUXCMD
\cite{matthews2023global}}\hskip0pt%DIFAUXCMD
, and thus we do not include de Hawaiian species declared extinct by the IUCN in $2024$.
}\DIFaddend For our analyses, we focused on two time periods: (i) the avifauna present in \DIFdelbegin \DIFdel{1500 }\DIFdelend \DIFaddbegin \DIFadd{$1500$ }\DIFaddend CE (the ``historic dataset'') and (ii) the current native avifauna (``extant dataset'').
...
}\DIFdelend \DIFaddbegin \DIFadd{- see supplementary figure 7; following the recommendations of \mbox{%DIFAUXCMD
\citealt{pigot2020macroevolutionary} }\hskip0pt%DIFAUXCMD
and the dimensionality selection method from \mbox{%DIFAUXCMD
...
\DIFdel{of trait space occupancy }\DIFdelend \DIFaddbegin \DIFadd{Although most metrics captured a decrease in metric score relative to the amount of removed data, some metrics resulted in a score increase }\DIFaddend (e.g. the \DIFdelbegin \DIFdel{functional }\DIFdelend \DIFaddbegin \DIFadd{Functional }\DIFaddend evenness based on \DIFaddbegin \DIFadd{a }\DIFaddend hypervolume for the \DIFdelbegin \DIFdel{equalizing }\DIFdelend \DIFaddbegin \DIFadd{equalising }\DIFaddend mechanism) or non-linear \DIFdelbegin \DIFdel{ones }\DIFdelend \DIFaddbegin \DIFadd{responses }\DIFaddend (e.g. Alpha diversity based on \DIFaddbegin \DIFadd{a }\DIFaddend dendrogram for the \DIFdelbegin \DIFdel{equalizing mechanism).
This raises an interesting point: the naive expectation that removing 80 }\DIFdelend \DIFaddbegin \DIFadd{equalising epresent distributions of metric scores; see supplementary tables 2 }\DIFaddend to \DIFdelbegin \DIFdel{20\% of the data would result in a reduction of trait space occupancy metric score (negative values)is often true but by no means always true}\DIFdelend \DIFaddbegin \DIFadd{5 and 6 to 17)}\DIFaddend .
Some cases resulted in an increase in metric score relative to random removals (positive values in Figure \ref{Fig:simulation_results})\DIFdelbegin \DIFdel{!
...
For the empirical data, we see again a range of different possible interpretations depending on the metric describing the pattern \DIFaddbegin \DIFadd{(Figure \ref{Fig:empirical_results}, Supplementary table 5 and 18)}\DIFaddend .
...
\subsection{\DIFadd{Capturing approximated mechanisms with different metrics}}

\DIFadd{We tested 12 trait space occupancy metrics on simulated and empirical datasets to assess how each metric captures patterns of trait space changes based on stressors approximating ecological and evolutionary mechanisms.
...
\DIFadd{For example, say one is interested in looking at changes in functional diversity using a divergence metric calculated with the probability density method (e.g. }\texttt{\DIFadd{TPD::REND}}\DIFadd{).
...
\DIFadd{Regarding the different approximated mechanisms, it is interesting to note that our different approximations lead to different general behaviours of the metrics: for example the range of variance in the metric values is greater for the equalising mechanism and smaller for the competition mechanism (albeit nearly always distinguishable from the null mechanism).
...
\DIFaddend More broadly, as can be seen from this empirical case, patterns are likely to be much less clear than with simulated data due to multiple stressors acting simultaneously.
...
For example, an interesting one could be ``Are the extinctions \DIFdelbegin \DIFdel{uniform }\DIFdelend \DIFaddbegin \DIFadd{spread uniformly in the trait space }\DIFaddend across all birds in Hawaii?".
...
, larger species)are more likely to have gone extinct and thus that functional diversity loss is larger than through random extinctions }\DIFdelend \DIFaddbegin \DIFadd{Importantly, it should always be borne in mind that mechanisms ought of course to be more complex in real world scenarios.
For example, evolutionary mechanisms can vary through time or across clades (and so patterns are often the results of multiple processes); and ecological mechanisms are often intertwined and work together to generate a pattern }\DIFaddend (e.g. \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citealt{sayol2021loss,Matthews2022}}\hskip0pt%DIFAUXCMD
).
...
\textit{\DIFadd{We have data of bird traits for native species in Hawaii through time (e.g. before and after human colonisation of the archipelago).
...
\begin{tabular}{lrr}
 ...
 \DIFaddFL{at 1500 }&  \DIFaddFL{63 }& \DIFaddFL{60.84 }\\ 
   \hline
\end{tabular}
...
\begin{tabular}{lrrrrrr}
 ...
 \DIFaddFL{at 1500 }&  \DIFaddFL{63 }& \DIFaddFL{1.12 }& \DIFaddFL{0.85 }& \DIFaddFL{0.94 }& \DIFaddFL{1.19 }& \DIFaddFL{1.34 }\\ 
   \hline
\end{tabular}
...
And also because functions of ecosystems\DIFdelbegin \DIFdel{or }\DIFdelend \DIFaddbegin \DIFadd{, or the }\DIFaddend organisms and niches within \DIFdelbegin \DIFdel{it }\DIFdelend \DIFaddbegin \DIFadd{them, }\DIFaddend are hard to capture/understand/define.
...
In other words, it is important to choose a disparity/dissimilarity/functional diversity metric \DIFdelbegin \DIFdel{depending }\DIFdelend \DIFaddbegin \DIFadd{based }\DIFaddend on the question and the data at hand rather than as a default option or solely based on a previous inspiring publication\DIFaddbegin \DIFadd{.
