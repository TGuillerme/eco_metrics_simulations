...
%DIF DEL main_ecography_R1.tex   Mon Jan 27 16:10:17 2025
%DIF ADD main_ecography_R2.tex   Sun Feb  2 13:43:10 2025
\usepackage{natbib}
...
We show that different metrics perform differently when trying to capture signal from different \DIFdelbegin \DIFdel{approximation }\DIFdelend \DIFaddbegin \DIFadd{approximations }\DIFaddend of common mechanisms relative to no mechanism at all (null).
For example\DIFaddbegin \DIFadd{, }\DIFaddend competition was harder to disentangle from the null mechanisms compared to facilitation in our simulations.
...
\DIFdelbegin \DIFdel{While the first part of this framework (``what?'') has been studied extensively in the last decade, this was often undertaken outside of the ``what, how and why'' framework.Here }\DIFdelend \DIFaddbegin \DIFadd{Although some previous work has been done in understanding patterns in the context of mechanisms and processes (e.g. \mbox{%DIFAUXCMD
\cite{novack2016general1,novack2016general2}}\hskip0pt%DIFAUXCMD
), }\DIFaddend we argue that \DIFaddbegin \DIFadd{often }\DIFaddend the pattern description \DIFdelbegin \DIFdel{, although the basis of any future study, often follows some }\DIFdelend \DIFaddbegin \DIFadd{follows some previously used }\DIFaddend framework without a proper evaluation of its adequacy to answer the how and why.
...
We expect that our ability to capture a pattern does not only \DIFdelbegin \DIFdel{depends }\DIFdelend \DIFaddbegin \DIFadd{depend }\DIFaddend on the choice of metric (what) but also on the process and mechanism at hand (how and why).
...
We show that, that with a fixed process and mechanisms approximation, the choice of the statistic to describe the pattern (the disparity or functional diversity metrics) has a great impact on the interpretation of the data\DIFaddbegin \DIFadd{, }\DIFaddend with the exact same data \DIFdelbegin \DIFdel{being sometimes }\DIFdelend \DIFaddbegin \DIFadd{sometimes being }\DIFaddend interpreted in opposite ways \DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{leading to sometimes opposite potential interpretations.
}


\begin{table}[htbp]
\caption{\DIFaddFL{Glossary and equivalence of terms used in this paper; adapted from \mbox{%DIFAUXCMD
\cite{guillerme2018disprity} }\hskip0pt%DIFAUXCMD
and \mbox{%DIFAUXCMD
\cite{mammola2021concepts}}\hskip0pt%DIFAUXCMD
.}}
\centering
\begin{tabular}{p{2.5cm}p{4cm}p{5cm}p{5cm}}
\DIFaddFL{Term in this paper }& \DIFaddFL{definition }& \DIFaddFL{in ecology }& \DIFaddFL{in macroevolution }\\
\hline
\DIFaddFL{Trait space }& \DIFaddFL{Matrix ($n \times d$) with a structural relation between rows and columns }& \DIFaddFL{Functional space, morphospace , etc. }& \DIFaddFL{Morphospace, trait space, etc.  }\\
\hline
\DIFaddFL{Observations }& \DIFaddFL{Rows ($n$) }& \DIFaddFL{Taxa, field sites, environments, etc. }& \DIFaddFL{Taxa, specimen, populations, elements, OTUs, etc. }\\
\hline
\DIFaddFL{Dimensions }& \DIFaddFL{Columns ($d$) }& \DIFaddFL{Traits, ordination scores, distances, etc. }& \DIFaddFL{Traits, ordination scores, distances, etc. }\\
\hline
\DIFaddFL{Traits }& \DIFaddFL{Columns subset ($b \times n$; $b \leq d$) }& \DIFaddFL{uni/multidimensional characteristic*; e.g. body mass, geographical location }& \DIFaddFL{uni/multidimensional characteristic; e.g. landmark coordinates }\\
\hline
\DIFaddFL{Group }& \DIFaddFL{Rows subset ($m \times d$; $m \leq n$) }& \DIFaddFL{Treatments, phylogenetic group (clade), etc. }& \DIFaddFL{Clades, geological stratum, etc. }\\
\hline
\DIFaddFL{Metric }& \DIFaddFL{Statistic (i.e. a measure) }& \DIFaddFL{Dissimilarity index or metric, hypervolume, functional diversity, etc. }& \DIFaddFL{Disparity metric or index }\\
\hline
\DIFaddFL{Stressors }& \DIFaddFL{An algorithm removing a proportion of the observations }& \DIFaddFL{Introduction of invasive species, change of landscape use, climate change, pollution, experimental design, etc. }& \DIFaddFL{Mass extinction, Tectonic change, climate change, etc. }\\
\hline
\DIFaddFL{Stressors' intensity }& \DIFaddFL{The amount of observations removed (percentages) }& \DIFaddFL{Proportion of species affected, proportion of temperature change, etc. }& \DIFaddFL{Severity of mass extinction, amount of co-occurring species, etc. }\\
\hline
\end{tabular}
\label{Tab:glossary}
\DIFaddFL{* Note that our definition of a trait is a generalisation of \mbox{%DIFAUXCMD
\cite{violle2007let,mcgill2006rebuilding,dawson2021traits}}\hskip0pt%DIFAUXCMD
's suggestions expanded to include traits beyond the ones common in functional ecology (e.g. also in macroevolution, palaeontology, microbiology, etc.).
}\end{table}
\DIFaddend 
...
First, we simulated \DIFdelbegin \DIFdel{a single }\DIFdelend \DIFaddbegin \DIFadd{multiple independent }\DIFaddend random time dependent \DIFdelbegin \DIFdel{trait }\DIFdelend \DIFaddbegin \DIFadd{traits }\DIFaddend (Brownian Motion) under a model where lineages only speciate (no extinction; i.e. a pure birth speciation model) until reaching 200 observations in \texttt{R} \citep{rcore} using \texttt{treats} \citep{guillerme2024treats}.
\DIFdelbegin \DIFdel{This trait was simulated either as a }\DIFdelend \DIFaddbegin \DIFadd{We simulated either }\DIFaddend 2, 4 or 8 \DIFdelbegin \DIFdel{dimensional trait, in other words a Brownian Motion simulated in a }\DIFdelend \DIFaddbegin \DIFadd{independent (uncorrelated) Brownian Motion traits (equivalent to }\DIFaddend 2, 4 or 8 \DIFdelbegin \DIFdel{dimensions}\DIFdelend \DIFaddbegin \DIFadd{dimensional traits)}\DIFaddend .
Note here that we refer to traits as any measurable aspect of an organism (\textit{sensu} \cite{mcgill2006rebuilding} - but see \cite{dawson2021traits} for nuances in what researchers perceive \DIFaddbegin \DIFadd{as traits}\DIFaddend ).
These aspects are often expressed in one dimension but can in fact be described in any number of dimensions.
For example, for \DIFaddbegin \DIFadd{a }\DIFaddend trait defined as ``leaf insertion angle'', this can measured in one dimension (an angle in degrees) or three dimensions (the same angle expressed as the trigonometric relation of three sides of a triangle).
These observations on which the traits are measured can represent any biological focal entities, e.g. tips in a phylogenetic tree, individuals, species, populations, OTUs, etc. (see \DIFdelbegin \DIFdel{Glossary in the supplementary materials, table 1}\DIFdelend \DIFaddbegin \DIFadd{Table \ref{Tab:glossary}}\DIFaddend ).
...
\DIFdelbegin \DIFdel{I.e. }\DIFdelend \DIFaddbegin \DIFadd{In other words, }\DIFaddend pairs of observations that share similar trait combinations are more \DIFdelbegin \DIFdel{likely to remain }\DIFdelend \DIFaddbegin \DIFadd{common in the trait space than observations with dissimilar trait combinations }\DIFaddend (i.e. observations adjacent to each other in the trait space \DIFaddbegin \DIFadd{are more common}\DIFaddend ).
...
In other words the centre of the radius $\rho'$ is the observation with the highest numerical value on all dimensions\DIFdelbegin \DIFdel{- }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend in a 2D representation this is the observation in the top right corner (with $\rho'_{20} > \rho'_{40} > \rho'_{60} > \rho'_{80}$).
...
This is done by selecting $B$ discrete categories to summarise the distribution - i.e. bandwidths - using Silverman's ``rule of thumb'' \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
Where $p$ is a factor increasing the scaling power of the algorithm (here we \DIFaddbegin \DIFadd{arbitrarily }\DIFaddend used $p=3$ \DIFaddbegin \DIFadd{which was a relatively low scaling value that still resulted in visible changes observable in 2D}\DIFaddend ).
...
This approximates our \textbf{competition mechanism}, where observations in dense regions of the trait space are more likely to be removed than in sparse regions of the trait space \DIFaddbegin \DIFadd{(or, inversely, observations are more likely to be ``preserved''/``surviving'' in less dense regions)}\DIFaddend .
We chose this algorithm to approximate a stressor that could increase the probability of extinction for observations that share \DIFdelbegin \DIFdel{traits combination}\DIFdelend \DIFaddbegin \DIFadd{trait combinations}\DIFaddend .
...
For example, ``increasing density'' and ``decreasing size'' stressors will lead to just what their names suggest if the observations are uniformly distributed (just \DIFaddbegin \DIFadd{an }\DIFaddend average increase in density for ``increasing density'' and just \DIFaddbegin \DIFadd{an }\DIFaddend overall decrease in size for the ``decreasing size'' stressors) but will have more or less similar effects (albeit not identical) in the case of a normal distribution (average increase in density and overall decrease in size for both ``increasing density'' and ``decreasing size'' stressors).
...
We used 171 replicates because that was empirically the smallest number of replicates required to reach a variance between replicates lower than 1\% across all metrics (i.e. any additional replicates beyond 171 added less \DIFdelbegin \DIFdel{that }\DIFdelend \DIFaddbegin \DIFadd{than }\DIFaddend 1\% extra variance).
...
However, this is not tackled here for both simplicity and to understand how \DIFdelbegin \DIFdel{they }\DIFdelend \DIFaddbegin \DIFadd{the stressors }\DIFaddend work in isolation. 
Therefore the prediction of how each different mechanism approximation would affect the metrics (Table \ref{Tab:mechanisms}) should be taken as a \DIFdelbegin \DIFdel{cautious }\DIFdelend \DIFaddbegin \DIFadd{cautionary }\DIFaddend guideline when dealing with empirical data where the distribution of the data might not be normal and where it could be affected by multiple mechanisms.
...
In each pipeline, the dimensions \DIFdelbegin \DIFdel{where }\DIFdelend \DIFaddbegin \DIFadd{were }\DIFaddend simulated as uncorrelated.
We limited our simulations to a relatively small number of dimensions due to the constraints of some of the metrics used (e.g., \texttt{TPD::TPDsMean} is only implemented for up to 4 dimensions; \citealt{carmona2019trait}\DIFdelbegin \DIFdel{- note that in the supplementary figures, we measured the }\texttt{\DIFdel{TPD}} %DIFAUXCMD
\DIFdel{metrics on only the four first dimensions}\DIFdelend ) but also to avoid dealing with the curse of dimensionality \citep{bellman1957dynamic}.
...
\hline
  ...
  Equalising fitness & Size change (``size'') & When observations with trait combinations that are more extreme (i.e. observations located away from the centre of the trait space) are disadvantaged due to some resource concentration gradient \citep{chesson2000mechanisms,barot2004mechanisms}. & Metrics mainly capturing changes in \DIFdelbeginFL \DIFdelFL{richness }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{richness* }\DIFaddendFL (but less in \DIFdelbeginFL \DIFdelFL{divergence }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{divergence* }\DIFaddendFL and not in \DIFdelbeginFL \DIFdelFL{regularity}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{regularity*}\DIFaddendFL ) \\

  Facilitation & Density change (``density'') & When observations with similar trait combinations (i.e. located close together in the trait space) are more likely to survive the stressor \citep{bruno2003inclusion, danet2024species}. In other words, this simulates the idea that some observations are more likely to be resilient if they are present in a community of observations (e.g. a community of species) with shared traits rather than the opposite. & Metrics capturing changes in \DIFdelbeginFL \DIFdelFL{regularity }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{regularity* }\DIFaddendFL (but not \DIFdelbeginFL \DIFdelFL{richness and divergence}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{richness* and divergence*}\DIFaddendFL ) \\

  Environmental filtering & Position change (``position'') & When an increase in observations' trait similarity happens through some strong abiotic constraints \citep{cornwell2006trait}. For example, some trait combinations become more and more unlikely due to some environmental constraints (i.e. some regions of the trait space become unsuitable). & Metrics capturing changes in position (and \DIFdelbeginFL \DIFdelFL{richness}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{richness*}\DIFaddendFL ) \\\\

  Competitive exclusion & Evenness change (``evenness'') & When functionally similar observations compete more intensively with one another than with functionally dissimilar observations (``competitive exclusion principle''; \citealt{hardin1960competitive}). At the extreme, it implies that only dissimilar observations will coexist (``limiting similarity principle''; \citealt{macarthur1967limiting}). & Metrics capturing changes in \DIFdelbeginFL \DIFdelFL{divergence }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{divergence* }\DIFaddendFL (but not \DIFdelbeginFL \DIFdelFL{richness and regularity}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{richness* and regularity*}\DIFaddendFL ) \\\\
...
\DIFdelendFL \DIFaddbeginFL \scriptsize{\textbf{Description of the stressors applied to the simulated data and the mechanism each approximated.} Here we distinguish between the biological mechanism we are trying to simulate and the algorithmic one we used to simulate it. We also provide a more detailed description of the stressor. Note that in biological data, we don't expect any of the mechanisms to act on communities alone (e.g. equalising fitness and facilitation can both act on the distribution of species traits). Nor do we expect their effects to be unidirectional (for example, equalising fitness can happen both by removing the edges or changing the position of a trait distribution), thus, the type of metric expected to recover the mechanism can be variable. These mechanisms serve as a simplified description of reality for the narrative purpose of this paper.
    * Richness, divergence and regularity are dimensions of functional metrics that have been proposed to respectively reflect the sum of differences, the average differences and how regular the differences are among observations \citep{pavoine2011measuring,tucker2017guide,mammola2021concepts}.
}
\DIFaddendFL }
...
\caption{\DIFdelbeginFL %DIFDELCMD < \scriptsize{Illustration of the simulation protocol used in this paper.
%DIFDELCMD < A) we simulated a pure birth-tree with a trait evolving under a Brownian Motion process until reaching 200 observations resulting in a trait space of 2, 4, or 8 dimensions.
%DIFDELCMD < The first plot on row A illustrates one such simulation in one dimension through time (with time on the horizontal axis and trait value for one dimension on the vertical axis).
%DIFDELCMD < The second plot on the same row shows the same results but with a two dimensional trait represented here.
%DIFDELCMD < We then applied different stressors to the resulting trait space
%DIFDELCMD < We represent the elements removed by the stressors in blue and the ones kept in the trait space in orange.
%DIFDELCMD < The first column illustrate the distribution of the elements in one dimension with the overlap between the removed (blue) and kept (orange) elements in brown (note we did not measure any metric on 1D spaces)
%DIFDELCMD < The second column illustrate the same distributions in 2 dimensions.
%DIFDELCMD < We did not illustrate the distributions in 4 or 8 dimensions here.
%DIFDELCMD < We used the following stressors:
%DIFDELCMD < B) \textbf{null mechanism}: randomly removing a proportion of the observations;
%DIFDELCMD < C) \textbf{approximating equalising mechanism}: removing observations on the edge of the distribution;
%DIFDELCMD < D) \textbf{approximating facilitation mechanism}: removing observations to reduce the distance between pairs of observations (increasing local density; decreasing evenness);
%DIFDELCMD < E) \textbf{approximating filtering mechanism}: removing observations from one extreme of the distribution;
%DIFDELCMD < F) \textbf{approximating competition mechanism}: proportionally removing observations from the centre of the distribution.
%DIFDELCMD < The expected changes indicate the main type of metric affect by the stressor (following the aspects in \citealt{mammola2021concepts}).
%DIFDELCMD < G) we presented the results of the simulations as the scaled metric scores for the approximated mechanisms (C to F) relative to the null mechanism. The dot represents the median metric score and the thick and dashed lines respectively the 50\% and 95\% confidence intervals.}%%%
...
\hline
Richness & Trait Probability Density & Functional richness & Sum of cells where trait probability density $> 0$ & \texttt{TPD::REND}\DIFaddbeginFL \DIFaddFL{* }\DIFaddendFL & \cite{carmona2019trait}\\
 & \cr 
...
\hline
Divergence & Trait Probability Density & Divergence & Overlap between the TPD function of a single observation and the TPD function of the whole set of observations & \texttt{TPD::REND}\DIFaddbeginFL \DIFaddFL{* }\DIFaddendFL & \cite{carmona2019trait}\\
 & \cr 
...
\hline
Regularity & Trait Probability Density & Functional evenness & An index of the spread of the trait distribution \DIFdelbeginFL \DIFdelFL{in }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{compared to }\DIFaddendFL an even spread of the distribution (e.g. the height of bins in a histogram) & \texttt{TPD::REND}\DIFaddbeginFL \DIFaddFL{* }\DIFaddendFL & \cite{carmona2019trait}\\
 & \cr 
 ...
 & \cr 
 & \DIFdelbeginFL \DIFdelFL{Distances }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Dendrogram }\DIFaddendFL & Functional evenness & Regularity of abundances and distances along the minimum spanning tree linking all observations & \texttt{FD::dbFD()\$FEve} & \cite{Laliberte2010FD,villeger2008new}\\
\hline
...
\label{Tab:metrics}
\DIFaddbeginFL \DIFaddFL{* The }\texttt{\DIFaddFL{TPD}} \DIFaddFL{package currently only allows calculations for trait spaces up to four dimensions. This metric was exclude from trait spaces with more than four dimensions.
...
In other words testing whether the metric scores relative to null removals \DIFdelbegin \DIFdel{where }\DIFdelend \DIFaddbegin \DIFadd{were }\DIFaddend linearly scaling with the amount of data removed.
...
We \DIFdelbegin \DIFdel{also added two marine species to the prehistoric species list (}\textit{\DIFdel{Haliaeetus albicilla}} %DIFAUXCMD
\DIFdel{and }\textit{\DIFdel{Pterodroma hypoleuca}}%DIFAUXCMD
\DIFdel{); we compiled missing trait data for these two extinct species from \mbox{%DIFAUXCMD
\cite{sayol2021loss}}\hskip0pt%DIFAUXCMD
.
We }\DIFdelend extracted traits for extant species from the AVONET database \citep{tobias2022avonet}.
...
Note that we used the extinct species data as presented in \cite{matthews2023global}, and thus we do not include \DIFdelbegin \DIFdel{de }\DIFdelend Hawaiian species declared extinct by the IUCN in $2024$.
...
%DIFDELCMD < Negative and positive values signify that the metric score for the stressor of interest is lower/higher than the one from the random stressor.
...
%DIFDELCMD < The grey dots and corresponding CI lines represent distributions of metric scores not clearly distinguishable from the random metric scores (paired t-test $p$ value $> 0.05$).
%DIFDELCMD < Grey lines in the background across the distributions of different removal amounts represent the fitted linear model ``centred and scaled metric score function of the amount of data removed''.
%DIFDELCMD < Dashed thin grey lines represent non-significant linear models (slope's $p$ value $> 0.05$).
%DIFDELCMD < The model results (including $R^{2}$ and t-statistics) are available in supplementary table 3.
...
Although most metrics captured a decrease in metric score relative to the amount of removed data, some metrics resulted in a score increase (e.g. the Functional evenness based on a hypervolume for the equalising mechanism\DIFaddbegin \DIFadd{; positive values in Figure \ref{Fig:simulation_results}}\DIFaddend ) or non-linear responses (e.g. Alpha diversity based on a dendrogram for the equalising \DIFdelbegin \DIFdel{epresent }\DIFdelend \DIFaddbegin \DIFadd{represent }\DIFaddend distributions of metric scores; see supplementary tables 2 to 5 and 6 to 17).
\DIFdelbegin \DIFdel{Some cases resulted in an increase in metric score relative to random removals (positive values in Figure \ref{Fig:simulation_results}).
}\DIFdelend Furthermore, although we predicted some more common changes in aspects of functional diversity for some specific mechanisms, our simulations show that most metrics under most aspects (richness, divergence or regularity) capture changes of trait space occupancy under any mechanism (see caveats section in the discussion).
...
%DIFDELCMD < Plain circles represent the metrics for the species present on Hawaii before 1500 (historic data; 46\% removal) and fainted squares for the species currently present on Hawaii (extant data; 69\% removal).
%DIFDELCMD < \textbf{NOTE: for the \texttt{TPD} metrics, the metric is calculated using the 4 first dimensions only.}
%DIFDELCMD < The model results (including $R^{2}$ and t-statistics) are available in supplementary table 4.
%DIFDELCMD < }%%%
...
However, some metrics (e.g. \DIFdelbegin \DIFdel{the }\DIFdelend Functional evenness based on the hypervolume) capture a pattern clearly greater than the one from the null mechanism.
This means that changes in metric values can be counter intuitive and should always \DIFdelbegin \DIFdel{been }\DIFdelend \DIFaddbegin \DIFadd{be }\DIFaddend interpreted in the context of the metric, data and question at hand.
\DIFaddbegin \DIFadd{Note also that, as mentioned above, different dimensionalities can have different effects on the metrics, depending on both the trait space and the metric nature \mbox{%DIFAUXCMD
\citep{bellman1957dynamic}}\hskip0pt%DIFAUXCMD
.
}\DIFaddend 
...
They can apply this metric to their trait space and get two relative metric scores of \DIFdelbegin \DIFdel{, say, }\DIFdelend $0.7$ and $0.65$ (pattern) for two levels of extinction (process).
...
Our results \DIFdelbegin \DIFdel{indicating }\DIFdelend \DIFaddbegin \DIFadd{show }\DIFaddend that the loss of  functional diversity \DIFdelbegin \DIFdel{(based on several metrics ) }\DIFdelend \DIFaddbegin \DIFadd{for several metrics }\DIFaddend was not more than expected by chance\DIFdelbegin \DIFdel{when focusing on all extinctions }\DIFdelend \DIFaddbegin \DIFadd{.
This }\DIFaddend could be due to the \DIFdelbegin \DIFdel{fact that although most global bird species are (or were, if accounting for anthropogenic extinctions) passerines (}\DIFdelend \DIFaddbegin \DIFadd{taxonomic distribution of the extinct birds: these Hawaiian data comprise }\DIFaddend approximatively $58$\% \DIFdelbegin \DIFdel{), }\DIFdelend \DIFaddbegin \DIFadd{passerines while }\DIFaddend the majority of \DIFdelbegin \DIFdel{global known anthropogenic extinctions (approx. }\DIFdelend \DIFaddbegin \DIFadd{anthropogenic extinctions have involved non-passerines (approximatively }\DIFaddend $75$\%)\DIFdelbegin \DIFdel{have been non-passerines, which }\DIFdelend \DIFaddbegin \DIFadd{.
Non-passerines }\DIFaddend tend to be larger and possess more unique morphological traits (thus resulting in relatively large reductions in functional diversity).
...
The simulated trait spaces in 2, 4 or 8 dimensions had the same variance on all dimensions (i.e. we used the same trait simulation process for all dimensions) whereas in empirical trait spaces, commonly generated using ordination techniques (e.g. PCA\DIFdelbegin \DIFdel{or PCO }\DIFdelend \DIFaddbegin \DIFadd{, PCO or PCoA}\DIFaddend ), the dimensions have by definition a decreasing variance.
...
In a higher number of dimensions (usually $>$10 but this is highly variable depending on the system under study), metrics results are \DIFdelbegin \DIFdel{harder }\DIFdelend \DIFaddbegin \DIFadd{more difficult }\DIFaddend to scale linearly.
This sometimes makes metric interpretation more chaotic \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{bellman1957dynamic} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{bellman1957dynamic} }\hskip0pt%DIFAUXCMD
}\DIFaddend in the sense that there is no direct relationship between the number of observations and dimensions that can be used to extrapolate results in higher dimensional datasets.
...
This definition of \DIFaddbegin \DIFadd{the }\DIFaddend null hypothesis is also appropriate for the empirical data if the question is the same.
...
Furthermore, the scaling of the metrics paired with the levels of removal (80\%, 60\%, 40\% and 20\%) creates an expected \DIFdelbegin \DIFdel{artifact }\DIFdelend \DIFaddbegin \DIFadd{artefactual }\DIFaddend similarity when comparing the random removals against the \DIFdelbegin \DIFdel{other ones}\DIFdelend \DIFaddbegin \DIFadd{removals due to stressors}\DIFaddend : when less data \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend available overall, the metrics are more likely to be similar (e.g. if removing 99\% of the data, we expect the random \DIFdelbegin \DIFdel{and }\DIFdelend removal metric score to be nearly identical to the non-random ones).

Importantly, it should always be borne in mind that mechanisms ought of course to be more complex in real world scenarios.
For example, evolutionary mechanisms can vary through time or across clades (and so patterns are often the \DIFdelbegin \DIFdel{results }\DIFdelend \DIFaddbegin \DIFadd{result }\DIFaddend of multiple processes); and ecological mechanisms are often intertwined and work together to generate a pattern (e.g. facilitation + competition; \citealt{danet2024species}), or counteract each others by operating on the trait space in opposite directions (e.g., competition + filtering; \citealt{mammola2024functional})\DIFaddbegin \DIFadd{.
}\DIFaddend 
...
\textit{We have data of bird traits for native species in Hawaii through time (e.g. \DIFdelbegin \DIFdel{before and after human colonisation of }\DIFdelend the \DIFdelbegin \DIFdel{archipelago}\DIFdelend \DIFaddbegin \DIFadd{birds known to be present in 1500 CE and those still extant today}\DIFaddend ).
We want to know how anthropogenic \DIFdelbegin \DIFdel{stresses }\DIFdelend \DIFaddbegin \DIFadd{stressors }\DIFaddend (e.g. hunting, invasive species, land use change) have affected the trait diversity of native Hawaiian birds.}

\begin{figure}[!htbp]
\centering
   \includegraphics[width=0.5\textwidth]{Figures/empirical_example.pdf}
\caption{Example of a \DIFdelbeginFL \DIFdelFL{traitspace }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{trait space }\DIFaddendFL based on the empirical data described above. \DIFaddbeginFL \DIFaddFL{Species in 1500' refers to extinct species that were believed to have been extant in 1500 CE.}\DIFaddendFL }
...
\begin{itemize}
    \item \textbf{What?} Choose the appropriate pattern that would capture changes through time in relation to a filtering mechanism. In our example, this could be a continuous metric that measures either changes in the trait space position or/and changes in the trait space \DIFdelbegin \DIFdel{side}\DIFdelend \DIFaddbegin \DIFadd{size}\DIFaddend . The idea being that filtering would remove either the edges or a specific portion of the trait space without affecting \DIFdelbegin \DIFdel{to }\DIFdelend many other parts of the trait space.
\end{itemize}


\subsubsection{Find the type of metrics of interest:}

Once \DIFdelbegin \DIFdel{the }\DIFdelend one has clearly defined the pattern they want to capture (\textbf{what}), we believe it \DIFdelbegin \DIFdel{gets easier to pick up the type of metricthat would be the most adequate}\DIFdelend \DIFaddbegin \DIFadd{becomes easier to select an appropriate metric}\DIFaddend .
For example, we can follow the suggestions from \cite{mammola2021concepts} \DIFaddbegin \DIFadd{highlighting which kind of methodological pipeline best supports which type of metric of interest }\DIFaddend (Table \ref{Tab:box2}).
...
Something else & Does the position of a group \DIFdelbeginFL \DIFdelFL{changes }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{change }\DIFaddendFL in space? & Yes \\
\end{tabular}
...
\subsubsection{\DIFdelbegin \DIFdel{Get }\DIFdelend \DIFaddbegin \DIFadd{Select }\DIFaddend the metric most appropriate to your data:}

\DIFdelbegin \DIFdel{So to }\DIFdelend \DIFaddbegin \DIFadd{To }\DIFaddend describe the pattern for our specific example question, we will be interested in size and position of the trait space.
...
Of course, these are example questions and are not \DIFdelbegin \DIFdel{necessary }\DIFdelend \DIFaddbegin \DIFadd{necessarily }\DIFaddend the only ones you should ask.
...
\section{Conclusion}
Different ecological and evolutionary processes or mechanisms do not always \DIFdelbegin \DIFdel{results }\DIFdelend \DIFaddbegin \DIFadd{result }\DIFaddend in different patterns.
