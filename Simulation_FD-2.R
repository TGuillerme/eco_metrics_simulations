...
####### 
#######  CARLOS: I ADD A COUPLE OF THINGS HERE, I WILL EXPLAIN IN THE EMAIL:
dissTraits <- as.matrix(gowdis(trait_space))
melodicRes <- melodic.rao(samp = t(data[,3:8]), dis = dissTraits)
melodic_Rao <- melodicRes$rao[1]
melodic_mpd <- melodicRes$mpd[1]
#######  #######  #######  #######  #######  #######  #######  #######  
...
cl <- makeSOCKcluster(n.par-2)
registerDoSNOW(cl)

#This for a progress bar
pb <- tkProgressBar(max = n.perm)
...
result <- foreach(p=1:n.perm,
  ...
  random_reduction      <- reduce.space(trait_space, type = "random", remove = remove_par)
  facilitation          <- reduce.space(trait_space, type = "density", remove = remove_par)
  equalizing_fitness    <- reduce.space(trait_space, type = "size", remove = remove_par)
  competitive_exclusion <- reduce.space(trait_space, type = "evenness", remove = remove_par, parameters = list(power = 3))
  filtering_reduction   <- reduce.space(trait_space, type = "position", remove = remove_par)
