...
```{r, echo = FALSE}
kable(null_table, caption = "Difference between the null metric and the stressed metric (raw) for each level of removal and each stressor", align = "c")
```
...
plot.metric.results(all_results, metric = "alpha", scale.method = "within", level.names = removals, col = cols)
```

### Dispersion

```{r, fig.height = 4, fig.width = 12}
plot.metric.results(all_results, metric = "dispersion", scale.method = "within", level.names = removals, col = cols)
```

### Evenness

```{r, fig.height = 4, fig.width = 12}
plot.metric.results(all_results, metric = "evenness", scale.method = "within", level.names = removals, col = cols, legend.pos = "bottomright")
```
