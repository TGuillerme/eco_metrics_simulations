...
```{r}
## Creating the function for simulating equalizing
simulate.equalizing <- make.simulation.pipeline(type = "equalizing")
```
...
For that we will use the custom `optim.replicate()` function which comes with one diagnosis function (checking if the variance for each metric is < 0.05) and one summarising (summarising the output of `simulate.equalizing()` for diagnosis) function
...
results <- optim.replicate(input.fun = simulate.equalizing,
                           diagnose = var,
...
analyse.replicates("../Data/Processed/sim_equalizing.rda") #126
analyse.replicates("../Data/Processed/sim_equalizing.rda") #163
```